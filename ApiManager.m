...
static NSString *const kContentListPath = @"/feeditem";
...
- (NSURLSessionDataTask *)getUsersWithRequestModel:(UserListRequestModel *)requestModel success:(void (^)(UserListResponseModel *responseModel))success failure:(void (^)(NSError *error))failure {
    ...
    return [self GET:kUsersListPath parameters:parametersWithKey progress:nil
...
- (NSURLSessionDataTask *)getContentWithRequestModel:(ContentListRequestModel *)requestModel success:(void (^)(ContentListResponseModel *responseModel))success failure:(void (^)(NSError *error))failure{
    ...
    return [self GET:kContentListPath parameters:parametersWithKey progress:nil
             success:^(NSURLSessionDataTask *task, id responseObject) {
                 NSDictionary *responseDictionary = (NSDictionary *)responseObject;
                 NSError *error;
                 ContentListResponseModel *list = [MTLJSONAdapter modelOfClass:ContentListResponseModel.class fromJSONDictionary:responseDictionary error:&error];
                 success(list);
             } failure:^(NSURLSessionDataTask *task, NSError *error) {
                 failure(error);
             }];
